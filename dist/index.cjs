...
var EventQueue = class {
  ...
  configure(apiBaseUrl) {
    const config = new import_mcpcat_api.Configuration({ basePath: apiBaseUrl });
    this.apiClient = new import_mcpcat_api.EventsApi(config);
  }
...
function setupInitializeTracing(highLevelServer) {
  ...
  if (originalInitializeHandler) {
    server.setRequestHandler(
      import_types2.InitializeRequestSchema,
      async (request, extra) => {
        const data = getServerTrackingData(server);
        if (!data) {
          writeToLog(
        ...
        let event = {
          ...
          eventType: import_mcpcat_api4.PublishEventRequestEventTypeEnum.mcpInitialize,
          parameters: {
            request,
            extra
          },
...
function setupToolCallTracing(server) {
  try {
    ...
    server.setRequestHandler(import_types2.CallToolRequestSchema, async (request, extra) => {
      const data = getServerTrackingData(server);
      if (!data) {
        writeToLog(
          "Warning: MCPCat is unable to find server tracking data. Please ensure you have called track(server, options) before using tool calls."
      ...
      let event = {
        ...
        eventType: import_mcpcat_api4.PublishEventRequestEventTypeEnum.mcpToolsCall,
        timestamp: /* @__PURE__ */ new Date(),
        redactionFn: data.options.redactSensitiveInformation
      };
      try {
...
var import_mcpcat_api5 = require("mcpcat-api");
    ...
    try {
      ...
      if (!data) {
        ...
        event = {
          ...
          eventType: import_mcpcat_api5.PublishEventRequestEventTypeEnum.mcpToolsCall,
          timestamp: startTime,
          redactionFn: data.options.redactSensitiveInformation
        };
        await handleIdentify(server, data, request, extra);
...
var TraceContext = class {
  getTraceId(sessionId) {
    ...
    return (0, import_crypto2.createHash)("sha256").update(sessionId).digest("hex").substring(0, 32);
  }
  getSpanId(eventId) {
    if (!eventId) {
      return (0, import_crypto2.randomBytes)(8).toString("hex");
    }
    return (0, import_crypto2.createHash)("sha256").update(eventId).digest("hex").substring(0, 16);
  }
...
var import_mcpcat_api6 = require("mcpcat-api");
var PostHogExporter = class {
  ...
  buildCaptureEvent(event) {
    ...
    if (event.resourceName) {
      ...
      if (event.eventType === import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpToolsCall) {
  ...
  mapEventType(eventType) {
    const mapping = {
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpToolsCall]: "mcp_tool_call",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpToolsList]: "mcp_tools_list",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpInitialize]: "mcp_initialize",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpResourcesRead]: "mcp_resource_read",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpResourcesList]: "mcp_resources_list",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpPromptsGet]: "mcp_prompt_get",
      [import_mcpcat_api6.PublishEventRequestEventTypeEnum.mcpPromptsList]: "mcp_prompts_list"
    };
...
var import_mcpcat_api7 = require("mcpcat-api");
var UmamiExporter = class {
  ...
  buildPayload(event) {
    ...
    if (event.resourceName) {
      ...
      if (event.eventType === import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpToolsCall) {
  ...
  mapEventType(eventType) {
    const mapping = {
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpToolsCall]: "mcp_tool_call",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpToolsList]: "mcp_tools_list",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpInitialize]: "mcp_initialize",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpResourcesRead]: "mcp_resource_read",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpResourcesList]: "mcp_resources_list",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpPromptsGet]: "mcp_prompt_get",
      [import_mcpcat_api7.PublishEventRequestEventTypeEnum.mcpPromptsList]: "mcp_prompts_list"
    };
    return mapping[eventType] || `mcp_${eventType.replace(/^mcp:/, "").replace(/\//g, "_")}`;
  }
};
